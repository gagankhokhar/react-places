...
export default class ImageGallery extends React.Component {
    ...
    componentWillMount() {
        ...
        firebase.database().ref('place').on('child_added', snapshot => {
            this.setState({
    ...
    handleUpload(event) {
        if (event && event.target && event.target.files.length) {
            ...
            const storageRef = firebase.storage().ref(`/place/${file.name}`);
            ...
            task.on('state_changed', snapshot => {
                ...
                const dbRef = firebase.database().ref('place');
                const newImage = dbRef.push().key;
                // newImage.set(record);
